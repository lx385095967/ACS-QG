...
class Trainer:
    ...
    def _train_epoch(self, epoch: int,slot_labels:list) -> Dict[str, float]:
        ...
        for batch in train_generator_tqdm:
            ...
            loss = self._batch_loss(batch, slot_labels,for_training=True)
    ...
    def train(self) -> Dict[str, Any]:
        ...
        for epoch in range(epoch_counter, self._num_epochs):
            ...
            train_metrics = self._train_epoch(epoch,slot_labels)
            #{'word-accuracy-overall': 0.44, 'question-accuracy': 0.0, 'partial-question-accuracy': 0.1, 'loss': 7.518}

            if self._validation_data is not None:
                with torch.no_grad():
                    # We have a validation set, so compute all the metrics on it.
                    val_loss, num_batches = self._validation_loss(slot_labels_val=slot_labels)
