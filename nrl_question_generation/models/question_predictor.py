...
class QuestionPredictor(Model):
    def __init__(self, vocab: Vocabulary,
        ...
        self.dim_hidden = dim_hidden#100
    ...
    def forward(self, 
                ...
                slot_labels_forward:list,
                **kwargs):
        span_mask = (labeled_spans[:, :, 0] >= 0).long()#tensor([[ 1,  1,  1,  1,  1,  1]])
        
        # span_slot_labels = []####################
        span_slot_labels_new = {}
        for i, n in enumerate(self.slot_labels):
            ...
            if 'span_slot_%s'%n in kwargs and kwargs['span_slot_%s'%n] is not None:
                ...
                span_slot_labels_new[n]=kwargs['span_slot_%s'%n] * span_mask
        if len(span_slot_labels_new) == 0:
            span_slot_labels_new = None


        ##############把原来的span_slot_labels###############
        span_slot_labels = []
        for i, n in enumerate(self.slot_labels):    
            if 'span_slot_%s'%n in kwargs and kwargs['span_slot_%s'%n] is not None:
                span_slot_labels.append(kwargs['span_slot_%s'%n] * span_mask)       
        ...
        ##################下面就是正式模型#########################
        embedded_text_input = self.embedding_dropout(self.text_field_embedder(text))#将输入进行embedding->torch.Size([1, 25, 1124])
        
        
        mask = get_text_field_mask(text)#就是输入的掩码(5,25)
        
        
        embedded_predicate_indicator = self.predicate_feature_embedding(predicate_indicator.long())#将谓词掩码也进行embedding->torch.Size([1, 25, 100])
        
        embedded_text_with_predicate_indicator = torch.cat([embedded_text_input, embedded_predicate_indicator], -1)#将两者进行拼接->torch.Size([1, 25, 1224])

        batch_size, sequence_length, embedding_dim_with_predicate_feature = embedded_text_with_predicate_indicator.size()

        if self.stacked_encoder.get_input_dim() != embedding_dim_with_predicate_feature:#四层的AugmentedLstm（StackedAlternatingLstm）
        ...
        encoded_text = self.stacked_encoder(embedded_text_with_predicate_indicator, mask)#torch.Size([5, 25, 300])
        
        #############################上面得到了每个token的隐向量之后###################################
        span_reps = self.span_extractor(encoded_text, labeled_spans, sequence_mask=mask, span_indices_mask = span_mask)#torch.Size([1, 6, 600])
        
        output_dict = {}
        
        slot_logits = self.question_generator(span_reps, slot_labels=span_slot_labels_new,encoded_text = encoded_text,mask = mask,slot_labels_forward=slot_labels_forward)
        #########################上面得到了每个槽的结果#################
        ##########把结果按照原本数据形式排列#############
        slot_logits_new=[]
        for i, n in enumerate(self.slot_labels):
            # Replace scores for padding and unk
            slot_logits[n][:,:,0:2] -= 9999999

            output_dict["slot_logits_%s"%n] = slot_logits[n]
            if n in slot_logits:
                slot_logits_new.append(slot_logits[n])
        slot_logits = slot_logits_new
        loss = None
